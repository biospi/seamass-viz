...
next_stream(double _mz_min, double _mz_max, double _rt_min, double _rt_max, double _max_counts)
{
	...
	double x_res = (mz_max - mz_min)/matDy.cols()/1.0033548378/60.0;
	double y_res = (rt_max - rt_min)/matDy.rows();
...
next_chunk(const vector<Coef>& cs)
{
	...
	for (int lx = 0; lx < nlx; lx++)
	{
		...
		if (DEBUG)
		{
			...
			cout << "t(Cy[" << ly0+matCy.size()-1 << "](" << matCy.back().rows() << "," << ix[lx] << ":" <<  ix[lx+1]-1 << "))" << endl;
	...
	SparseMatrix<double> matDx(matDy.cols(), matCx.back().cols());
	{
		SparseMatrix<double> matBx(matDy.cols(), matCx.back().rows());
		...
		for (int j = 0; j < matBx.cols(); j++)
		{
			...
			double x0f = matDy.cols() * (mz0 - mz_min) / (mz_max - mz_min);
			double x1f = matDy.cols() * (mz1 - mz_min) / (mz_max - mz_min);

			// bounding box intersection
			int x0 = (int) floor(x0f); x0 = x0 > 0 ? x0 : 0;
			int x1 = (int) ceil(x1f); x1 = x1 < matDy.cols() ? x1 : matDy.cols();			
		...
		if (DEBUG)
		{
			...
			cout << "Cx[" << lx0+matCx.size()-1 << "](" << matCx.back().rows() << "," << matCx.back().cols() << ")" << endl;
	...
	{
		SparseMatrix<double> matBy(matDy.rows(), matDx.cols());
		...
		for (int j = 0; j < matBy.cols(); j++)
		{
			...
			// project into viewport x/y space
			double y0f = matDy.rows() * (rt0 - rt_min) / (rt_max - rt_min);
			double y1f = matDy.rows() * (rt1 - rt_min) / (rt_max - rt_min);

			// bounding box intersection
			int y0 = (int) floor(y0f); y0 = y0 > 0 ? y0 : 0;
			int y1 = (int) ceil(y1f); y1 = y1 < matDy.rows() ? y1 : matDy.rows();
	...
	filesystem::path pngfile_path = stream_path / pngfile_oss.str();
	writer.write(pngfile_path, matDy, max_counts);
